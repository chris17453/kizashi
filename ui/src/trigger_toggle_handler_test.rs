...
fn sample_trigger(id: Uuid, tenant_id: Uuid, enabled: bool) -> TriggerDefinition {
    TriggerDefinition {
        id,
        tenant_id,
...
#[tokio::test]
async fn toggling_an_enabled_trigger_disables_it() {
    ...
    triggers_client.full_triggers.lock().unwrap().push(sample_trigger(trigger_id, tenant_id, true));
    ...
    let response = router(state)
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!("/triggers/{trigger_id}/toggle"))
...
#[tokio::test]
async fn viewer_role_cannot_toggle_a_trigger() {
    ...
    let response = router(state)
        .oneshot(
            Request::builder()
                .method("POST")
                .uri(format!("/triggers/{}/toggle", Uuid::new_v4()))
                .header("cookie", format!("kizashi_session={session_id}"))
                .body(Body::empty())
                .unwrap(),
        )
