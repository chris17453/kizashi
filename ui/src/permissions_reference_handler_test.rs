...
fn router(state: AppState) -> Router {
    Router::new().route("/security/permissions", get(get_permissions_reference)).with_state(state)
}

async fn state_with_session(role: Role) -> (AppState, String) {
...
#[tokio::test]
async fn any_authenticated_role_can_view_the_reference() {
    for role in [Role::Viewer, Role::Operator, Role::Admin] {
        let (state, session_id) = state_with_session(role).await;
        ...
        assert_eq!(response.status(), StatusCode::OK, "role {role:?} should be able to view it");
    }
}

#[tokio::test]
async fn shows_every_documented_area() {
    let (state, session_id) = state_with_session(Role::Viewer).await;
    ...
    for area in [
        ...
        assert!(body.contains(area), "missing area: {area}");
    }
}

#[tokio::test]
async fn redirects_to_login_when_not_signed_in() {
    let (state, _session_id) = state_with_session(Role::Viewer).await;

    let response = router(state)
        .oneshot(Request::builder().uri("/security/permissions").body(Body::empty()).unwrap())
        .await
        .unwrap();
