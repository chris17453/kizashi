...
fn apply_branding_to_html_replaces_the_product_name_and_accent_color() {
    let branding = Branding {
        product_name: Some("Acme Support".to_string()),
        logo_url: None,
...
#[test]
fn apply_branding_to_html_escapes_html_special_characters_in_the_product_name() {
    let branding = Branding {
        product_name: Some("A & B <Corp>".to_string()),
        logo_url: None,
        accent_color: None,
    };
    ...
    assert!(rewritten.contains("A &amp; B &lt;Corp&gt;"));
}

#[test]
fn apply_branding_to_html_leaves_html_unchanged_when_fields_are_unset() {
    let branding = Branding::default();

    let rewritten = apply_branding_to_html(SAMPLE_HTML, &branding);

    assert_eq!(rewritten, SAMPLE_HTML);
}

fn router(state: AppState) -> Router {
    async fn page() -> axum::response::Response {
        ([("content-type", "text/html")], SAMPLE_HTML).into_response()
    }
    use axum::response::IntoResponse;
    Router::new()
        .route("/page", get(page))
        .layer(axum::middleware::from_fn_with_state(state.clone(), apply_branding))
        .with_state(state)
}

fn sample_state(branding_client: InMemoryBrandingClient) -> (AppState, Arc<InMemorySessionStore>) {
    let session_store = Arc::new(InMemorySessionStore::default());
    let state = AppState {
        session_store: session_store.clone(),
        auth_client: Arc::new(InMemoryAuthClient::default()),
        branding_client: Arc::new(branding_client),
...
#[tokio::test]
async fn rewrites_the_page_when_the_session_tenant_has_branding_configured() {
    let branding_client = InMemoryBrandingClient::default();
    *branding_client.branding.lock().unwrap() = Some(Branding {
        product_name: Some("Acme Support".to_string()),
        logo_url: None,
        accent_color: None,
    });
    let (state, session_store) = sample_state(branding_client);
    ...
    assert!(body.contains("Acme Support"));
}

#[tokio::test]
async fn leaves_the_page_unchanged_with_no_session_cookie() {
    let branding_client = InMemoryBrandingClient::default();
    *branding_client.branding.lock().unwrap() = Some(Branding {
        product_name: Some("Acme Support".to_string()),
        logo_url: None,
    ...
    let (state, _session_store) = sample_state(branding_client);
    ...
    let bytes = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
    let body = String::from_utf8(bytes.to_vec()).unwrap();
    assert!(body.contains(r#"<span class="brand-name">Kizashi</span>"#));
}

#[tokio::test]
async fn leaves_the_page_unchanged_when_the_tenant_has_no_branding_configured() {
    let (state, session_store) = sample_state(InMemoryBrandingClient::default());
    let session_id = session_store
        .create(Session {
            bearer_token: "tok".to_string(),
            tenant_id: Uuid::new_v4(),
