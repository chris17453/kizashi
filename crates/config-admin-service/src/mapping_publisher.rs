...
/// Publishes `mapping.changed` on every mapping create/update, mirroring ADR-0018's
/// `trigger.changed` pattern — closes the gap ADR-0010 originally flagged for both
/// trigger-engine and normalization-service ("not yet the operational source of truth") but
/// only ADR-0018 actually closed, for triggers. Without this, an operator editing a Field
/// Mapping through the Console UI has zero effect on the running normalization pipeline until
/// a manual DB write or restart.
#[async_trait]
