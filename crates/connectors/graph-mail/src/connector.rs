...
/// Polls a mailbox's messages via Microsoft Graph (`GET /users/{id}/messages`), authenticated
/// with the tenant's own Entra app registration via the client-credentials flow ADR-0003
/// specifies (app-only auth can't use `/me`, hence the explicit `user_id`). A fresh token is
/// fetched on every poll — simplest correct behavior for a one-shot CronJob invocation
/// (ADR-0013) rather than caching a token across a process that only ever calls `poll` once.
pub struct GraphMailConnector {
    connector_id: String,
...
#[async_trait]
impl Connector for GraphMailConnector {
    ...
    async fn poll(&self, tenant_id: uuid::Uuid) -> Result<Vec<RawRecord>, ConnectorError> {
        ...
        let response = self
            .client
            .get(format!("{}/users/{}/messages", self.graph_base_url, self.user_id))
