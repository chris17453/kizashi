...
async fn spawn_stub_graph_server(
    ...
    let app = if status.is_success() {
        Router::new().route("/users/:user_id/messages", get(handler)).with_state(messages)
    } else {
        Router::new().route("/users/:user_id/messages", get(error_handler)).with_state(status)
    };
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    tokio::spawn(async move {
...
async fn polls_a_real_graph_server_and_maps_messages_to_raw_records() {
    ...
    let messages = serde_json::json!([{"id": "1", "subject": "hi"}]);
    ...
    assert_eq!(records[0].connector_id, "graph-mail");
