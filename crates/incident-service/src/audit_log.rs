...
pub struct AuditLogEntry {
    ...
    /// The real user identity behind the change — sourced from the `X-Username` header, same
    /// trust boundary as config-admin-service's audit log.
    pub actor: String,
...
/// (CLAUDE.md §5) — same shape as config-admin-service's `record_audit_entry`.
