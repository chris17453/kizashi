...
impl EventPublisher for FailingEventPublisher {
    async fn publish_record_ingested(&self, _record: &RawRecord) -> Result<(), PublishError> {
...
async fn in_memory_publisher_records_published_events() {
    ...
    publisher.publish_record_ingested(&record).await.unwrap();
...
async fn failing_publisher_returns_bus_error() {
    ...
    let err = publisher.publish_record_ingested(&record).await.unwrap_err();
