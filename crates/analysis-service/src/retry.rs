...
pub const RETRY_COUNT_HEADER: &str = "x-analysis-retry-count";

/// A permanently-failing message (e.g. a dead test tenant with no reachable AI backend) is
/// dead-lettered after this many attempts instead of being requeued forever, so it can't starve
/// other tenants' messages in the same single-consumer FIFO queue.
