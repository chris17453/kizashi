...
pub struct InMemorySessionClient {
    pub minted: Mutex<Vec<(Uuid, String)>>,
}
...
impl SessionClient for InMemorySessionClient {
    async fn mint_session(
        ...
        self.minted.lock().unwrap().push((tenant_id, label.to_string()));
...
async fn in_memory_client_records_minted_sessions() {
    ...
    let token = client.mint_session(tenant_id, "local-login").await.unwrap();
...
async fn http_client_parses_a_successful_response() {
    ...
    let token = client.mint_session(Uuid::new_v4(), "local-login").await.unwrap();
...
async fn http_client_returns_rejected_on_error_status() {
    ...
    let err = client.mint_session(Uuid::new_v4(), "local-login").await.unwrap_err();
