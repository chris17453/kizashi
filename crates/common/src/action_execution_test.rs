...
fn new_starts_pending() {
    ...
    assert_eq!(exec.tenant_id, tenant_id);
...
fn retry_creates_new_row_referencing_same_trigger_and_event() {
    ...
    let event_id = Uuid::new_v4();
    let original =
        ActionExecution::new(tenant_id, trigger_id, event_id, ActionType::Webhook, json!({}));
    ...
    assert_eq!(retried.tenant_id, original.tenant_id);
