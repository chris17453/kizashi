use backup_service::{
    build_router, AppState, PostgresBackupRunRepository, ProcessPgDumpRunner, S3BackupStore,
};
use std::sync::Arc;
...
async fn build_s3_client() -> aws_sdk_s3::Client {
    ...
    let s3_config = aws_sdk_s3::config::Builder::from(&shared_config)
        ...
        // tolerates it too, so it's safe to always set (ADR-0011, mirrored by ADR-0055).
...
async fn main() {
    ...
    let store = S3BackupStore::new(s3_client, bucket);
    store.ensure_bucket().await.expect("failed to ensure backup bucket exists");

    let state = AppState {
        run_repository: Arc::new(PostgresBackupRunRepository::new(pool)),
        store: Arc::new(store),
        dump_runner: Arc::new(ProcessPgDumpRunner::new(database_url)),
        internal_secret,
    };
