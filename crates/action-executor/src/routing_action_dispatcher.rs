...
/// Routes each action to the dispatcher that can actually fulfill it (ADR-0023): an
/// `ActionType::Email` action whose config carries `smtp_host` is a real SMTP send via
/// `SmtpActionDispatcher`; everything else (including `Email` actions still configured as a
/// webhook, for backward compatibility with ADR-0007's original "everything is HTTP POST"
/// model) goes to `HttpActionDispatcher`. This is the dispatcher `main.rs` actually wires up.
...
impl RoutingActionDispatcher {
    pub fn new(egress_proxy_url: Option<String>) -> Self {
        Self {
            http: HttpActionDispatcher::new(egress_proxy_url),
