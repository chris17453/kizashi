...
pub const RETRY_COUNT_HEADER: &str = "x-action-executor-retry-count";

/// A permanently-failing message is dead-lettered after this many attempts instead of being
/// requeued forever, so it can't starve other tenants' messages in the same single-consumer
/// FIFO queue -- same rationale and threshold as analysis-service's identical mechanism.
